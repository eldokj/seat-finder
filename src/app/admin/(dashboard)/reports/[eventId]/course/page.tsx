...
import { groupByCourse, needsDraftWatermark, filterSeatedRowsByYearTerm, distinctYearsAndTerms } from "@/lib/validation/reports";
...
export default async function CourseWiseReportPage(props: PageProps<"/admin/reports/[eventId]/course">) {
  ...
  const groups = groupByCourse(filterSeatedRowsByYearTerm(allRows, { year: yearFilter, term: termFilter }));
  ...
  return (
    <div className="max-w-4xl">
      <ReportToolbar
        ...
        exportHref={`/api/admin/reports/${eventId}/course/export${exportQuery ? `?${exportQuery}` : ""}`}
      />
      <ReportYearTermFilter
        action={`/admin/reports/${eventId}/course`}
        years={years}
        terms={terms}
      ...
      <ReportLetterhead title="Course-wise Report" examDate={event.examDate} session={event.session} />
      <GroupedReportTable groups={groups} groupLabel="Course" />
    </div>
  );
}
